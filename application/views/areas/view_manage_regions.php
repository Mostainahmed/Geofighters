<section class="content">
    <div class="container-fluid">
        ...
        <div class="row pt-3">
            ...
            <div class="col-sm-6">
				<a href="javascript:void(0)" onclick="view_regions_modal();" class="btn btn-sm btn-primary addemployeebtn " data-toggle="modal" data-target="#modal_employee_add"><i class="fas fa-plus fa-sm"></i> Add New Region</a>
			</div>	
        </div>
        <div class="row pt-0">
            <div class="col-12">
                <!-- need to change here in order to mark the nav item-->
                <table style="width:100%" class="table_regions table-hover table-bordered table-sm text-center">
                </table>
            </div>
        </div>
    </div>
...
<script>
var sl = 0;
var selected_region_dataset = [];
var current_region_dataset = [];
$('.table_regions').DataTable({
	processing: true,
	serverSide: true,
	stateSave : true,
	ordering : true,
	...
	ajax: {
		url: "<?php echo base_url(); ?>area/get_region_list_serverside",
		type: 'POST',
		dataFilter: function(data){
			//console.log(data);
			sl = JSON.parse(data)['start'];
			current_region_dataset = JSON.parse(data)['data'];
			return data;
		},
		error: function(err){
			console.log(err);
		...
		},
		{
			...
			"data": {
			 ...
			 render: function (data, type, service) {
			 	...
			 	let edit_button = '<a class="" href="javascript:void(0)" onclick="update_region(\''+data.region_id+'\');"><button type="button" class="btn btn-warning btn-xs"><i class="fas fa-edit"></i> Edit</button></a>'
...
<script>
    ...
    $('#navlink_region_management').addClass('active');
</script>