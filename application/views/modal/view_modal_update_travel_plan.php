<div class="modal fade" id="modal_update_travel_plan">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="frm_travel_plan_update" role="form">
...
<script>
  function update_travel_plan(userid) {
    ...
    // $('#txt_user_update_id').val(selected_user_dataset['itemcode']);

    $('#modal_update_travel_plan').modal('show');
}

$('#frm_travel_plan_update').on('submit', function(e) {
    e.preventDefault();
    var form_travel_plan_update = new FormData($('#' + e.target.id)[0]);
    var values = $(this).serialize();
    console.log(values);

    Swal.fire({
        title: 'Do you want to Update This Travel Plan?',
        type: 'question',
        ...
        if (result.value) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data == 1) {
                        Swal.fire({
                            title: 'Travel Plan updated successfully',
                            type: 'success',
                        }).then((data) => {
                            location.reload();
                        });
